...
export default function AgentScreen() {
  ...
  const filteredAgens = agenData.filter(
    (agen) =>
  ...
  const renderService = ({ item }: { item: any }) => (
    <View style={[styles.serviceCard, { borderLeftColor: item.color }]}>
      <View style={styles.serviceContent}>
        ...
        <View
          style={[styles.limitBadge, { backgroundColor: `${item.color}15` }]}
        >
          <Text style={[styles.limitText, { color: item.color }]}>
            {item.limit}
          </Text>
        </View>
      </View>
          ...
          <View style={styles.servicesContainer}>
            ...
            {item.layanan.length > 3 && (
              <TouchableOpacity
                style={[
                  styles.moreButton,
                  { backgroundColor: isExpanded ? "#FF660015" : "#f0f0f0" },
                ]}
            ...
            <View style={styles.headerContent}>
              <View style={styles.heroSection}>
                <Text style={styles.heroSubtitle}>
                  Temukan agen BNI terdekat untuk layanan perbankan lengkap
        ...
        ListFooterComponent={() => (
          <View>
            ...
            {filteredAgens.map((item) => (
              <View key={item.id}>{renderAgen({ item })}</View>
            ))}
          </View>
        )}
